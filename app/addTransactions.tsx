...
export default function transactions() {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (
				res.status === 200 &&
				res.data.message.includes("transaction added") &&
				res.data.id
			) {
				...
				<View className="flex-1 pb-10">
					<View className="gap-8 flex-1">
						<View className="bg-[#E2E8F0] h-16 rounded-xl gap-4 px-2  items-center flex flex-row justify-between">
							<Pressable
								{...(selectedType === "expense" && {
									style: { elevation: 10, backgroundColor: "#13EC5B" },
								})}
							...
							>
								<Text
									style={{
										color: selectedType === "expense" ? "black" : "#4D5B6F",
									}}
							...
							>
								<Text
									style={{
										color: selectedType === "income" ? "black" : "#4D5B6F",
									}}
									className="text-lg font-semibold"
								>
									Income
						...
						<View className="flex-1 gap-4">
							<CategorySelector
								...
								ToggleItem={setSelectedCategoryId}
							/>
							<Pressable
								onPress={() => setIsSelectingDate(true)}
								className="justify-between bg-white rounded-2xl shadow-[0_3px_5px_rgb(0,0,0,0.1)] flex flex-row px-4 h-20 items-center"
							>
							...
							<DatePickerModal
								...
								date={selectedDate}
								onConfirm={(params) => {
									setSelectedDate(params?.date ?? new Date());
					...
					<View>
						<Pressable
							...
							className="bg-[#13EC5B] h-14 justify-center items-center rounded-xl mt-4"
						>
...
const CategorySelector = ({
	...
	ToggleItem: (categoryId: number) => void;
}) => {
	return (
		<Dialog>
			<Dialog.Trigger>
                    ...
                    flex justify-center items-center
                      `}
										>
											<Icon
												name={"check-line"}
												className=" font-bold self-center text-900"
												size={15}
												color={"white"}
											/>
										</View>
