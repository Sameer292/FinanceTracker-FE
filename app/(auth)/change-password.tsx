...
const ChangePassword = () => {
  ...
  const ChangePasswordSchema = object({
    current_Password: string({ error: 'Please enter your current password' }).min(1, { error: 'Please enter your current password' }),
    new_Password: string({ error: 'Please enter your new password' }).min(8, { error: 'Password must be at least 8 characters long' }),
    confirm_new_Password: string({ error: 'Please enter your new password' }).min(8, { error: 'Password must be at least 8 characters long' }),
  ...
  const onSubmit = async (data: ChangePasswordSchemaType) => {
    ...
    changePasswordMutation.mutate({ current_password: properData.current_Password, new_password: properData.new_Password })
  }
      ...
      <View className="flex-1 items-center gap-8">
        <View className="w-full flex-1 pb-6 justify-between">
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <View className="justify-center gap-2">
                    <Text
                      className="text-lg text-[#1E1E1E] font-nunito-medium"
                    >
                      Current Password
                    </Text>
                    <SecureTextField
          ...
          <View className="gap-3 ">
            <Pressable disabled={changePasswordMutation.isPending} className="bg-[#07D19D] py-4 rounded-xl items-center" onPress={methods.handleSubmit(onSubmit)}>
              {changePasswordMutation.isPending ? <ActivityIndicator color="white" />
                :
            ...
            <Pressable onPress={() => router.back()} className="border border-[#118AB2] py-4 rounded-xl items-center">
