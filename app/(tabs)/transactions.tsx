...
export default function transactions() {
	...
	const [transactionType, setTransactionType] =
		useState<TransactionTypeSelect>();
	...
	const filteredTransactions = MyTransactions?.transactions?.filter((tx) => {
		...
		const matchType =
			...
			tx.transaction_type === transactionType.value;
					...
					>
						{today.length > 0 && (
							<View className="flex-1 gap-4">
								<Text className="text-center text-2xl text-900 font-bold text-[#6B7280]">
									Today
								</Text>
								<View className="flex-1 gap-4">
						...
						{today.length === 0 &&
							yesterday.length === 0 &&
							lastWeek.length === 0 && <Text>Add transaction</Text>}
						{lastWeek.length > 0 && (
							<View className="flex-1 gap-4">
								<Text className="text-center text-2xl text-900 font-bold text-[#6B7280]">
									Last 7 days
