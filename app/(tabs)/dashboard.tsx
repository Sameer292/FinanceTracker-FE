		...
		<View className="gap-4 flex-1 items-center pt-5 px-4 justify-center bg-white relative">
			...
			<View className="w-full gap-3">
				<View className="flex-row justify-between border-[0.5] items-center border-[#A9DFBF] bg-linear-to-r from-[#F5F7FA] to-[#F1F9F4] rounded-lg pl-4.5 pr-5">
					<View className="py-4">
						<Text className="text-[#8395A7] text-lg font-nunito-semibold">
							Total Income
						</Text>
						<Text className="text-[#37474F] text-xl font-nunito-bold">
			...
			<View className="flex-1 gap-4 mt-4 w-full">
				<View className="w-full justify-between flex-row items-center">
					...
					{latestTransactions &&
						latestTransactions?.transactions.length > 0 && (
							<TouchableOpacity
								className="p-4"
								onPress={() => console.log("See all")}
							>
								<Text className="text-[#8395A7] font-nunito-semibold text-sm">
									See All
								</Text>
							</TouchableOpacity>
						)}
				...
				{error && (
					<View className="items-center gap-2 mt-10">
						...
						<Pressable
							onPress={() => refetchRecentTransactions()}
							className="px-4 py-2 bg-[#06D6A0] rounded-lg"
						>
					...
					<FlatList
						...
						ListEmptyComponent={() => (
							<View className="items-center gap-2 justify-center flex-1">
								...
								<Text className="text-center text-x font-nunito-semibold">
									No transactions yet.
								</Text>
								<Text className="text-[#8395A7] text-lg text-center font-nunito-regular">
									Start tracking your spending by adding your first expense or
									income.
								</Text>
							</View>
						)}
