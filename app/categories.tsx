	...
	return (
		<View className="flex-1 gap-4 pt-5 bg-white ">
			...
			<View className="pb-4 px-4 gap-4 justify-center items-center flex">
				...
				<View className="w-full flex-row items-center gap-2 border border-[#D9E3E8] rounded-lg  px-3">
					...
					<TextInput
						...
						placeholder="Search"
						className="font-nunito-regular flex-1 text-lg text-[#37474F]"
					/>
					{searchText.length > 0 && (
					...
					<FlatList
						...
						renderItem={({ item }) => (
							<Pressable
								...
								className="justify-center w-full items-center gap-4 flex-row py-4 px-3 border border-[#D9E3E8] rounded-2xl"
							>
								<View
									style={{ backgroundColor: item.color, borderRadius: "100%" }}
									className={`size-15 justify-center items-center`}
								>
									<Icon
						...
						ListEmptyComponent={() => {
							if (
								...
								return (
									<View className="items-center gap-2 justify-center flex-1 -mt-50">
										<Icon name="menu-search-fill" color="#06D6A0" size={24} />
										<Text className="text-center text-xl font-nunito-semibold">
											No matches found.
										</Text>
										<Text className="text-[#8395A7] text-lg text-center font-nunito-regular">
											We couldn't find anything for "{searchText}". Check your
											spelling or try a different category.
										</Text>
									</View>
								);
							} else {
								return (
									<View className="items-center gap-2 justify-center flex-1 -mt-50">
										...
										<Text className="text-[#8395A7] text-lg text-center font-nunito-regular">
											Create categories to organize your spending and see where
											your money goes.
										</Text>
									</View>
								);
			...
			<View className="absolute bottom-14 right-4">
				<Link
					href="/addCategories"
					className="bg-[#06D6A0] border border-[#D9E3E8] rounded-full p-3"
				>
