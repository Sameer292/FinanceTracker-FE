...
export function splitByDateRanges<T extends TransactionType>(
	data: T[],
): { today: T[]; yesterday: T[]; lastWeek: T[] } {
...
export function getTopCategories(
	...
	// convert to array
	const categoryTotals = Object.entries(expenseTotals).map(
		([catId, amount]) => ({
	...
	const topCategories = top.map(
		(t) => categories.find((c) => c.id === t.categoryId) as Category,
	);
