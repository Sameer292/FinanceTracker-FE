		...
		<>
			<Dialog className="flex-1 h-14">
				<Dialog.Trigger
					...
					className={`${isOpen || selectedItems.length ? "bg-[#C8F5D6]" : "bg-[#EEF0F1]"} flex-1 items-center rounded-full`}
				>
					<View className="flex-1 w-full items-center flex-row rounded-full h-14 justify-between px-6 ">
						<Text
							className={`${isOpen || selectedItems?.length ? "text-[#15c34f]" : ""} text-lg font-bold `}
						>
							{title}
						</Text>
						{isOpen ? (
							...
							<Feather
								name={"chevron-down"}
								color={`${selectedItems.length && "#15c34f"}`}
								size={20}
							/>
						)}
					</View>
				...
				<Dialog.Portal className="px-4 -mt-90 ">
					...
					<Dialog.Content
						isSwipeable={false}
						className="border border-[#F3F4F6] rounded-2xl bg-white shadow-[1px_1px_25px_5px_rgba(0,0,0,0.29)]"
					>
						{/* Header */}
						<View className="flex-row items-center justify-between border-b border-[#F3F4F6] px-4 py-3">
							...
							<Pressable onPress={handleSelection}>
								<Text className="text-md font-medium text-[#15c34f]">
